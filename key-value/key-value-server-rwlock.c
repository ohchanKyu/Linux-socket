...
const char *get_value(const char *key) {
    pthread_rwlock_rdlock(&store_rwlock); 
...
int main() {
    ...
    if (pthread_rwlock_init(&store_rwlock, NULL) != 0) {
        perror("Read-write lock init failed");
        close(server_fd);
        return -1;
    }
    ...
    pthread_rwlock_destroy(&store_rwlock);
    close(server_fd);
